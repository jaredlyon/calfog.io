...
def analyze_dataset(file_path):
    ...
    class_weight = {0: total / (2.0 * n_neg), 1: total / (2.0 * n_pos)} if n_pos > 0 else None
    
    model = build_temporal_cnn((sequence_length, X_train_seq.shape[2]))
    ...
    y_pred_train_proba = model.predict(X_train_seq, verbose=0).flatten()
    y_pred_test_proba = model.predict(X_test_seq, verbose=0).flatten()
    ...
    train_auc = roc_auc_score(y_train_seq, y_pred_train_proba) if len(np.unique(y_train_seq)) > 1 else 0
    test_auc = roc_auc_score(y_test_seq, y_pred_test_proba) if len(np.unique(y_test_seq)) > 1 else 0
    
    first_conv_weights = model.layers[0].get_weights()[0]
    importance_values = np.mean(np.abs(first_conv_weights), axis=(0, 2))
...
def main():
    ...
    output_dir = Path("models/temporal_cnn")
    output_dir.mkdir(parents=True, exist_ok=True)
    output_file = output_dir / f"temporal_cnn_results.txt"
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        for result in all_results:
            ...
            f.write(f"\n  Feature Importance (sorted by first-layer weight magnitude, {len(without['feature_importance'])} features):\n")
            ...
            f.write(f"\n  Feature Importance (sorted by first-layer weight magnitude, {len(with_aqi['feature_importance'])} features):\n")
            ...
            f.write(f"\n  Feature Importance (sorted by first-layer weight magnitude, {len(without_reduced['feature_importance'])} features):\n")
