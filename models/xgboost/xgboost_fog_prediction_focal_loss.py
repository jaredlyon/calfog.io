...
def focal_loss_objective(preds, dtrain, alpha=0.25, gamma=1.0):
    ...
    focal_weight_pos = alpha * np.power(1 - p, gamma)
    focal_weight_neg = (1 - alpha) * np.power(p, gamma)
    
    grad = np.where(
        labels == 1,
        focal_weight_pos * (p - 1),
        focal_weight_neg * p
    )

    hess = np.where(
        labels == 1,
        focal_weight_pos * p * (1 - p),
        focal_weight_neg * p * (1 - p)
    )
...
def analyze_dataset(file_path):
    ...
    focal_alpha = min(n_pos / (n_pos + n_neg), 0.75)  
    focal_gamma = 1.0  
    
    def focal_loss_obj(preds, dtrain):
        return focal_loss_objective(preds, dtrain, alpha=focal_alpha, gamma=focal_gamma)
    ...
    def focal_loss_eval(preds, dtrain):
        labels = dtrain.get_label()
    ...
    y_pred_train_raw = model.predict(dtrain)
    y_pred_test_raw = model.predict(dtest)
    
    y_pred_train_proba = 1.0 / (1.0 + np.exp(-y_pred_train_raw))
    y_pred_test_proba = 1.0 / (1.0 + np.exp(-y_pred_test_raw))
...
def main():
    ...
    output_file = output_dir / f"xgboost_focal_loss_results.txt"
    ...
    for without_aqi_file in without_aqi_files:
        ...
        if without_results and with_results and without_reduced_results:
            ...
            print(f"Location {location_num} processed successfully")
        else:
            print(f"Location {location_num} skipped (missing data)")
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("XGBoost with Focal Loss - Fog Prediction Results\n")
        f.write("=" * 80 + "\n")
        f.write(f"Focal Loss Parameters: alpha=adaptive (based on class balance), gamma=1.0\n")
        f.write(f"Focal Loss focuses on hard-to-classify examples to improve minority class detection\n")
        f.write("=" * 80 + "\n\n")
